...
require_once 'inc/bs_Plugin.php';
    ...
    function init() {
        ...
        if ( get_user_option('rich_editing') == 'true' ) {
        	...
        	foreach (glob(dirname(__FILE__)."/inc/*.php") as $filename) {
        		if( basename($filename) == "bs_Plugin.php")
        			continue;
        		$pluginName = basename($filename, ".php");
        		$pluginName = substr($pluginName, strlen("bs_"));
        		$this->plugins[] = new BS_Plugin( $pluginName );
        	}
        }
    }
                ...
                <table class="form-table">
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            ...
                            <label>
                                <input 
                                    name="bs_options[chk_default_options_<?php echo $plugin->getName(); ?>]"
                                    type="checkbox"
                                    value="1"
                                    <?php if( isset($options['chk_default_options_' . $plugin->getName() ]) ) { checked('1', $options['chk_default_options_'.$plugin->getName()] ); } ?>
